...
class TargetRegion:
    ...
    def get_bounds(self):
        ...
        for cluster in clusters:
            min_bounds = torch.maximum(torch.min(cluster, dim=0).values - self.min_sample_distance / 2.0, self.global_bounds[0])
            max_bounds = torch.minimum(torch.max(cluster, dim=0).values + self.min_sample_distance / 2.0, self.global_bounds[1])
